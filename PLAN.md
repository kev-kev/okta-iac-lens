...
- [x] **Final M4 step — comprehensive security review (2026-07-03). Result: NO critical/high findings; one informational.**
  ...
  - **Informational (the one finding):** the tenant subdomain `integrator-1546176` is committed in `seed/main.tf`. Not a credential — org URLs are user-facing by design, and this is a disposable free test tenant — but it does name a live endpoint in a public repo. Option if desired: parameterize `org_name` via a Terraform variable. Left as-is pending a decision; fine to ship.
