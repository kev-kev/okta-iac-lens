...
- **E — verify + lock.** **Automated: ✅** full suite **207 passed | 0 expected-fail** (red #7 flipped, `.fails` marker gone); `npm run build` + `web:typecheck` clean; grep confirms **no** `weaker-than-peers`/`gateStrength`/`WEAK_GATE_MULT` literal remains and **no** `it.fails(` marker survives. **Human checks (still pending — need the free Integrator tenant, no code dependency):** live seed trace (`trace --user test.user@…`) shows Salesforce via individual assignment and still matches the console; **hidden-app probe** — assign an app to the test user with "hide from end-user dashboard" set, confirm whether appLinks returns it, and if it doesn't, add a one-line blind-spot caveat to the trace output and record the finding here. Security note: appLinks is a new GET on the users scope — re-confirm read-only (`smoke --verify-readonly` still 403s on the write probe; appLinks is a plain GET so it's unaffected).
...
- Viewer individual-assignment channel — **trimmed from M13 Phase D (2026-07-15); now live here.** Thread `directApps` (live appLinks diff) through `/api/user-membership` + the vite middleware, and render individual apps as app nodes with no group edge (a distinct "individual" visual channel) in `user-access-graph.ts`/`buildUserAccessGraph` + `UserTracePanel`. CLI+core already carry the data; this is the browser surface only.
