...
- **Rejected (with cause, revisitable):** semantic-zoom/HIERARCHY-clustered overviews — Okta data has no natural hierarchy to cluster on, and they converge to needing focus views at the leaves anyway. **Qualifier (2026-07-06):** COHORT aggregation is different and adopted — grouping by computed dimensions (connectivity band, policy, coverage bucket) into a few meta-cards per lane with count-carrying ribbons works without any hierarchy, and is the query-first landing above the threshold. Adjacency matrix still noted as the canonical dense-bipartite encoding if assignment-density analysis ever becomes a first-class task.
